...
Write-Host "Unreal Engine Editor Launcher" -ForegroundColor Green
...
$editorPath = Join-Path $ueDir "Engine\Binaries\Win64\UnrealEditor.exe"
if (-not (Test-Path $editorPath)) {
    Write-Host "ERROR: UnrealEditor.exe not found at: $editorPath" -ForegroundColor Red
    Read-Host "Press Enter to exit"
    exit 1
}
...
if ($AdditionalParams -and $AdditionalParams.Length -gt 0) {
    Write-Host "Additional parameters: $($AdditionalParams -join ' ')"
}

# Launch the Unreal Editor with the project and any additional parameters
Write-Host "Opening Unreal Editor..."
$startArgs = @("`"$uprojectPath`"")
if ($AdditionalParams -and $AdditionalParams.Length -gt 0) {
    $startArgs += $AdditionalParams
}
Start-Process -FilePath $editorPath -ArgumentList $startArgs
